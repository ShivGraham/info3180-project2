...
def get_profile(user_id):
    ...
    if users is None:
        ...
        # Fetch user's posts
        user = {'id': users.id, 'username': users.username, 'firstname': users.firstname, 'lastname': users.lastname, 'email': users.email, 'location': users.location, 'biography': users.biography, 'profile_photo': users.profile_photo, 'joined_on'}
        if posts is None:
