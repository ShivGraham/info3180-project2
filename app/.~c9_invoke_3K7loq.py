...
def all_posts():
    ...
    for post in posts:
        likes = getLikes(post.user_id)
        
        posts_list += [{'id': post.id, 'user_id': post.user_id, 'photo': post.photo, 'caption': post.caption, 'created_on': post.created_on, 'likes': likes}]
    ...
    posts_list += [{'id': 2, 'user_id': 2, 'username': '''photo': "sana.jpg", 'caption': "Sana", 'created_on': "2018-04-06 13:15:00", 'likes': 100}]
...
def posts(user_id):
    ...
    for post in posts:
        ...
        posts_list += [{'id': post.id, 'user_id': post.user_id, 'username': uname, 'photo': post.photo, 'description': post.caption}]
...
def follow(user_id):
    ...
    current_user_id = Users.query.filter_by(id=g.id).first()
    
    query = text("INSERT into Follows(id,user_id,follower_id) values(id+1,user_id,current_user_id)")
...
def get_followers(user_id):
    ...
    '''query = text("SELECT COUNT(DISTINCT follower_id) FROM Follows WHERE Follows.user_id = user_id ")
    follower_count = db.session.get_bind().execute(query)
    
    info = {'followers': follower_count }'''
    
        
    return  #json.dumps([dict(r) for r in follower_count])
...
def likes(post_id):
    ...
    query = text("INSERT into Likes(id,user_id,post_id) values(id+1,current_user_id,post_id)")
    db.session.execute(query)
    db.session.commit()
    
    #Get the current number of likes
    likes = getLikes(current_user_id)
    
    info = {'message': "Post liked!","likes" : likes }
